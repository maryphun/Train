...
- Combat control correction confirmed 2026-09-19: the player controls 桃香, not the monster. This explicit correction overrides the monster-player perspective in `戦闘案` slides 7–10. The monster responds automatically; the player-facing controls and result labels must use the heroine's perspective.
- Combat resources confirmed 2026-09-19: use both a turn limit labeled `残された時間` and monster HP labeled `体力`. The countdown remains turn-based. The heroine action values and automatic response pattern below are provisional, using the user's earlier authorization for customizable assumptions.
- Unity CLI refers to the Editor's built-in command-line tools.
- Japanese UI/dialogue text uses the existing `KiwiMaru-Regular SDF` TextMesh Pro asset and its bundled Kiwi Maru source font. On 2026-09-19, the user requested a fix for missing basic Japanese characters; the asset's dynamic atlas now allows multiple pages so supported characters can be added as needed.
...
- Implemented combat loop, corrected 2026-09-19: the player chooses a heroine skill, then the monster automatically responds; this consumes one turn. Monster HP reaching zero wins for 桃香 before any response. Energy reaching zero loses for 桃香. The existing time-expiry outcome is retained provisionally as monster retreat/heroine victory. Monster skills cycle in list order. Editable sample heroine actions attack monster HP, halve incoming damage for one response, or recover Energy up to its initial value. Portrait stages can change the sprite and add turns on accumulated damage thresholds. All numerical values remain editable examples, not approved balancing.
...
- Unresolved combat design: final heroine actions and their balance, final status mapping, and whether different monsters have special actions beyond configured skills. The three combat damage accumulators remain battle-time values, not persistent character statuses. Outcome multipliers now explicitly name heroine victory/defeat; the sample values preserve the prior damage-to-status scaling.
